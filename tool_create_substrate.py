...
def calc_matrices_triangle(R):
    ...
    # NNN along x (like tool_create_hex/circ)
...
# --- Sinusoidal triangular substrate
# Non normalised
def lj(x, eps=1, sig=1):
    return 4*eps*((sig/x)**12-(sig/x)**6)

#d, rc = 200, sig/5
#def fdmap (xx, d=1, rc=1):
#    return 1/(1+np.exp(-d*(xx/rc-1)))

def particle_en_lj(pos, pos_torque, basis, a, b, sigma, epsilon, d0, u, u_inv):
    """Calculate energy, forces and torque on each particle.

    Substrate energy is modelled as a lattice of LJ wells.
    """
    # REMEMBER: minimum of lj is at 2^(1/6)*sigma
    # TODO: sigma and epsilon as Nbasis,Npos matrix accounting for different atomic species
    en = np.zeros(shape=(pos.shape[0]))
    F = np.zeros(shape=(pos.shape[0],2))
    tau = np.zeros(shape=(pos.shape[0]))
    ...
    for r in basis:
        ...
        rho = (posR[tail]-a)/(b-a) # Reduce coordinate rho in [0,1]
        # TODO: implement D2 damping
        ftail = (1 - 10*rho**3 + 15*rho**4 - 6*rho**5)  # Damping f(x) -> [1,0] f(0)=1 f(1)=0
        dftail = (-30*rho**2 + 60*rho**3 - 30*rho**4)/(b-a) # Derivative of f
        #ftail, dftail = np.ones(posR[tail].shape),  np.zeros(posR[tail].shape)

        # Add z offset
        posR = np.sqrt(posR**2+d0**2)
        Rtail = posR[tail] # position in tail
        # Energy
        en[bulk] += lj(posR[bulk], epsilon, sigma) # Bulk. Somethigng like epsilon[ir], sigma[ir]
        en[tail] += lj(Rtail, epsilon, sigma)*ftail # Tail
        # Forces bulk F = -dE/dr
        #bulk = np.logical_and(posR<=a, posR != 0) # Exclude singular point in origin where F=0
        Fbulk_rad = 24*epsilon/posR[bulk]*(2*(sigma/posR[bulk])**12-(sigma/posR[bulk])**6)
        F[bulk, 0] = Fbulk_rad*pospp[bulk,0]/posR[bulk]
        F[bulk, 1] = Fbulk_rad*pospp[bulk,1]/posR[bulk]
        # Forces tail F = -d(E*f)/dr = - (E'*f + E*f')
        Ftail_rad = 24*epsilon/posR[tail]*(2*(sigma/posR[tail])**12-(sigma/posR[tail])**6)
        f1 = -en[tail]*dftail # E f'
        f2 = ftail*Ftail_rad # E' f
        F[tail, 0] += (f1+f2) * pospp[tail,0]/posR[tail]
        F[tail, 1] += (f1+f2) * pospp[tail,1]/posR[tail]
        # Torque
        tau += np.cross(pos-r-pos_torque, F)
...
def calc_en_lj(pos, pos_torque, basis, a, b, sigma, epsilon, d0, u, u_inv):
    """Calculate energy, forces and torque on CM.

    Substrate energy is modelled as a lattice of LJ."""
    en, F, tau = particle_en_lj(pos, pos_torque, basis, a, b, sigma, epsilon, d0, u, u_inv)
    return np.sum(en), np.sum(F, axis=0), np.sum(tau)
# ----------------------
...
def substrate_from_params(params):
    ...
    if well_shape == 'gaussian':
        ...
        pen_func, en_func = particle_en_lj, calc_en_lj
        sigma, d0, a, b = params['sigma'], params['d0'], params['a'], params['b']
        u, u_inv = calc_matrices_bvect(params['b1'], params['b2'])
        en_inputs = [basis, a, b, sigma, epsilon, d0, u, u_inv]
